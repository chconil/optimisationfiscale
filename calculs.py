...
class OptimisationRemunerationSARL:
    ...
    def calculer_scenario(self, remuneration_gerance, per_montant=0, madelin_montant=0, girardin_montant=0):
        ...
        revenu_imposable_apres_per = revenu_imposable - per_deduction
        
        # Madelin TNS : déduction supplémentaire
        madelin_deduction = min(madelin_montant, min(revenu_imposable_apres_per * 0.10, self.plafond_madelin))
        revenu_imposable_final = revenu_imposable_apres_per - madelin_deduction
        
        resultats['per_deduction'] = per_deduction
        resultats['madelin_deduction'] = madelin_deduction
    ...
    def optimiser_avec_niches(self, pas=5000, min_salaire=0, max_salaire=None, per_max=None, madelin_max=None, girardin_max=None):
        """Trouve la rémunération optimale en incluant les optimisations fiscales"""
        if max_salaire is None:
            max_salaire = self.resultat_avant_remuneration
        if per_max is None:
    ...
    def afficher_resultat(self, resultat):
        ...
        if 'optimisations' in resultat and any(resultat['optimisations'][k] > 0 for k in ['per', 'madelin', 'girardin']):
            ...
            print(f"💰 ÉCONOMIES D'IMPÔT TOTALES : {resultat['optimisations']['economies_ir']:,.0f}€")
        ...
        if 'madelin_deduction' in resultat and resultat['madelin_deduction'] > 0:
            print(f"  - Déduction Madelin : {resultat['madelin_deduction']:,.0f}€")
    ...
    def graphique_comparaison_optimisations(self, tous_scenarios):
        ...
        for i, scenario in enumerate(meilleurs_par_strategie):
            ...
            print(f"{marker}{nom_court:<42} {scenario['total_net']:>10,.0f}€ {scenario['remuneration_brute']:>12,.0f}€ {scenario['optimisations']['economies_ir']:>12,.0f}€")
