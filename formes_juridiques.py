...
class Microentreprise(OptimisationFiscale):
    ...
    def calculer_scenario(self, chiffre_affaires, type_activite='BIC', per_montant=0, **kwargs):
        ...
        # Net final
        net_final = chiffre_affaires - cotisations_sociales - ir - per_montant
        resultats['net_final'] = net_final
        
        # Taux de prélèvement global
        total_prelevements = cotisations_sociales + ir + per_montant
    ...
    def optimiser(self, type_activite='BIC', pas=5000, per_max=None, **kwargs):
        ...
        # Test de différents CA jusqu'au seuil
        for ca in range(pas, seuil + 1, pas):
            scenarios_ca = []
            
            # Sans optimisation
            scenario_base = self.calculer_scenario(ca, type_activite, 0)
            scenarios_ca.append(scenario_base)
            
            # Avec PER
            scenario_per = self.calculer_scenario(ca, type_activite, per_max)
            scenarios_ca.append(scenario_per)
            
            # Garder le meilleur pour ce CA
            meilleur_ca = max(scenarios_ca, key=lambda x: x.get('net_final', 0))
            tous_scenarios.append(meilleur_ca)
            
            if meilleur_scenario is None or meilleur_ca['net_final'] > meilleur_scenario['net_final']:
                meilleur_scenario = meilleur_ca
